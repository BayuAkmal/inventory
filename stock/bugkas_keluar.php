...
<?php 
    ...
    if(isset($_POST['update'])){
        $id = $_POST['id']; //iddata
        $idy = $_POST['idy']; //idsaldo
        $jumlahkeluar = $_POST['jumlahkeluar'];
        $penerima = $_POST['penerima'];
        $keterangan = $_POST['keterangan'];
        $tanggal = $_POST['tanggal'];

        $lihatjumlah = mysqli_query($conn,"select * from skas where idy='$idy'"); //lihat saldo saat ini
        $kassnya = mysqli_fetch_array($lihatjumlah); //ambil datanya
        $kasskrg = $kasnya['jumlah'];//jumlah saldo skrg
        ...
        if($jumlahkeluar >= $qtyskrg){
            ...
            $kurangijumlah = $kasskrg-$hitungselisih;

            $queryx = mysqli_query($conn,"update skas set jumlah='$kurangijumlah' where idy='$idy'");
            ...
            if ($updatedata1 && $queryx){
                ...
                <meta http-equiv='refresh' content='1; url= s_kaskeluar.php'/>  ";
                } else { echo "<div class='alert alert-warning'>
                    <strong>Failed!</strong> Redirecting you back in 3 seconds.
                </div>
                <meta http-equiv='refresh' content='3; url= skas_keluar.php'/> ";
                };

        } else {
            //ternyata inputan baru lebih kecil jumlah keluarnya, maka tambahi lagi saldo
            $hitungselisih = $qtyskrg-$jumlah;
            $tambahikas = $kasskrg+$hitungselisih;

            $query1 = mysqli_query($conn,"update skas set jumlah='$tambahikas' where idy='$idy'");

            $updatedata = mysqli_query($conn,"update skas_keluar set tgl='$tanggal', jumlah='$jumlah', penerima='$penerima', keterangan='$keterangan' where id='$id'");
            ...
            if ($query1 && $updatedata){
                ...
                <meta http-equiv='refresh' content='1; url= skas_keluar.php'/>  ";
                } else { echo "<div class='alert alert-warning'>
                    <strong>Failed!</strong> Redirecting you back in 3 seconds.
                </div>
                <meta http-equiv='refresh' content='3; url= skas_keluar.php'/> ";
    ...
    if(isset($_POST['hapus'])){
        ...
        $lihatstock = mysqli_query($conn,"select * from skas where idy='$idy'"); //lihat saldo saat ini
        $kasnya = mysqli_fetch_array($lihatjumlah); //ambil datanya
        $kasskrg = $kasnya['jumlahkeluar'];//jumlah saldo skrg

        $lihatdataskrg = mysqli_query($conn,"select * from skas_keluar where id='$id'"); //lihat qty saat ini
        $preqtyskrg = mysqli_fetch_array($lihatdataskrg); 
        $qtyskrg = $preqtyskrg['jumlahkeluar'];//jumlah skrg

        $adjustjumlah = $stockskrg+$qtyskrg;
                                ...
                                <div class="market-status-table mt-4">
                                    <div class="table-responsive">
										 <table id="dataTable3" class="table table-hover"><thead class="thead-dark">
											...
											<?php 
											$brg=mysqli_query($conn,"SELECT * FROM skas_keluar sb, skas st where st.idy=sb.idy ORDER BY id DESC");
											$no=1;
											while($b=mysqli_fetch_array($brg)){
                                                ...
                                                <div class="modal fade" id="edit<?=$id;?>">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <input type="number" id="jumlah" name="jumlahkeluar" class="form-control" value="<?php echo $b['jumlahkeluar'] ?>">
                                                    ...
                                                    <div class="modal fade" id="del<?=$id;?>">
                                                        ...
                                                        <form method="post">
                                                            <!-- Modal Header -->
                                                            <div class="modal-header">
                                                            <h4 class="modal-title">Hapus Transaksi<?php echo $b['nama']?></h4>
                                                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                                                            </div>
                                                            
                                                            <!-- Modal body -->
                                                            <div class="modal-body">
                                                            Apakah Anda yakin ingin menghapus transaksi ini dari daftar kas?
                                                            <br>
									...
									<a href="exporkaskeluar.php" target="_blank" class="btn btn-info">Export Data</a>
                                </div>
                            </div>
                        </div>
                    </div>
	...
	<!-- modal input -->
			<div id="myModal" class="modal fade">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<h4 class="modal-title">Input Kas Keluar / Pinjam</h4>
						</div>
						<div class="modal-body">
							<form action="kas_keluar_act.php" method="post">
								...
								<div class="form-group">
									<label>Nama Jenis Uang</label>
									<select name="saldo" class="custom-select form-control">
									<option selected>Pilih Saldo</option>
									<?php
									$det=mysqli_query($conn,"select * from skas order by nama ASC")or die(mysqli_error());
