...
<?php 
    ...
    if(isset($_POST['update'])){
        ...
        $lihatdataskrg = mysqli_query($conn,"select * from skas_masuk where id='$id'"); //lihat qty saat ini
        $preqtyskrg = mysqli_fetch_array($lihatdataskrg); 
        $qtyskrg = $preqtyskrg['jumlahmasuk'];//jumlah skrg

        if($jumlahmasuk >= $qtyskrg){ //
            $hitungselisih = $jumlahmasuk-$qtyskrg;
            $tambahikas = $kasskrg+$hitungselisih;

            $queryx = mysqli_query($conn,"update skas set jumlah='$tambahikas' where idy='$idy'");
            $updatedata1 = mysqli_query($conn,"update skas_masuk set tgl='$tanggal',jumlahmasuk='$jumlahmasuk', keterangan='$keterangan' where id='$id'");
            ...
            if ($updatedata1 && $queryx){
                ...
                <meta http-equiv='refresh' content='1; url= kas_masuk.php'/>  ";
                } else { echo "<div class='alert alert-warning'>
                    <strong>Failed!</strong> Redirecting you back in 3 seconds.
                </div>
                <meta http-equiv='refresh' content='3; url= kas_masuk.php'/> ";
                };

        } else {
            //ternyata inputan baru lebih kecil jumlah masuknya, maka kurangi lagi jumlahnya
            $hitungselisih = $qtyskrg-$jumlahmasuk;
            $kurangikas = $kasskrg-$hitungselisih;

            // $query1 = mysqli_query($conn,"update skas set jumlah='$kurangijumlah' where idy='$idy'");
            $query1 = mysqli_query($conn,"update skas set jumlah='$kurangikas' where idy='$idy'");

            $updatedata = mysqli_query($conn,"update skas_masuk set tgl='$tanggal', jumlahmasuk='$jumlahmasuk', keterangan='$keterangan' where id='$id'");
    ...
    if(isset($_POST['hapus'])){
        ...
        $jumlahnya = mysqli_fetch_array($lihatjumlah); //ambil datanya
        $jumlahskrg = $jumlahnya['jumlah'];//jumlah saldonya skrg
        ...
        $del = mysqli_query($conn,"delete from skas_masuk where id='$id'");
                    ...
                    <nav>
                        <ul class="metismenu" id="menu">
							...
							<li class="active">
                                ...
                                <ul class="active">
                                    <li class="active"><a href="kas_masuk.php">Kas Masuk</a></li>
                                ...
                                <div class="market-status-table mt-4">
                                    <div class="table-responsive">
										 <table id="dataTable3" class="table table-hover"><thead class="thead-dark">
											...
											<?php 
											$brg=mysqli_query($conn,"SELECT * from skas_masuk sb, skas st where st.idy=sb.idy order by sb.id DESC");
											$no=1;
											while($b=mysqli_fetch_array($brg)){
												...
												<tr>
													...
													<td>Rp.<?php echo $b['jumlahmasuk'] ?></td>
                                                ...
                                                <div class="modal fade" id="edit<?=$id;?>">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <input type="number" id="jumlahmasuk" name="jumlahmasuk" class="form-control" value="<?php echo $b['jumlahmasuk'] ?>"disabled>
                                                    ...
                                                    <div class="modal fade" id="del<?=$id;?>">
                                                        ...
                                                        <form method="post">
                                                            <!-- Modal Header -->
                                                            <div class="modal-header">
                                                            <h4 class="modal-title">Hapus Data Kas Masuk<?php echo $b['nama']?></h4>
                                                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                                                            </div>
									...
									<a href="exportkasmasuk.php" target="_blank" class="btn btn-info">Export Data</a>
                                </div>
                            </div>
                        </div>
                    </div>
			...
			<div id="myModal" class="modal fade">
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form action="kas_masuk_act.php" method="post">
